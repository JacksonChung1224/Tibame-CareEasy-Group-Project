...
### Added
- 🚀 **機構端個案連動管理 (任務卡 D)**：
  - **頂層檢視切換**：於機構端 (`/institution`) 頂部新增「月底核銷作業」與「個案連動管理」雙視圖切換，將原先 Header 右上角的全域「邀請家屬連動」按鈕整併入此頁。
  - **個案連動生命週期管理**：實作 Mock 個案列表，完整支援「未邀請」、「邀請中」、「已連動」三態管理。提供產生邀請碼、複製邀請碼、解除連動（自訂確認彈窗）、以及向連動家屬「推送本月紀錄」等功能。
  - **機制敘事與體驗呼應**：連動頁頂部新增機制說明橫幅，並在核銷流程步驟 2.5 加入 `✓ 確認後的紀錄將同步顯示於已連動家屬的照護日誌`，貫徹「一筆紀錄，三種價值」的產品機制。

- 🚀 **官方核銷格式對接 (任務卡 E)**：
  - **官方匯入格式支援**：實作 `scheduleImport.js` 純函式解析器，支援精準擷取 11 欄官方格式，自動尋找表頭並忽略多餘欄位；加入缺失必要欄位的整批阻擋與欄位型別驗證。
  - **真實上傳實作**：於機構端儀表板恢復 XLSX 原生檔案選擇上傳，介面支援顯示匯入成功筆數與異常警示清單，不阻擋正常資料。
  - **比對邏輯 (`reconcile.js`) 升級**：改用「身分證+日期+代碼」精準比對，支援 `qty`、`price` 等欄位輸出，補齊 `price * qty` 小計邏輯；匯入單價若與系統 `BA_MAP` 不符，觸發 `priceWarning` 註記但不阻擋。
  - **核銷報表匯出優化**：精確匯出符合官方 11 欄格式的 Sheet 1「核銷明細」，不加入任何自訂欄位；將異常（D1~D4）及待補價/警示項目獨立匯出至 Sheet 2「異常附表」供內部稽核。
  - **核銷總計看板**：步驟 4 新增 UI 合計卡，明確區分補助小計、自費小計、總額與異常/待補價筆數。

- 🚀 **三端體驗優化 (任務卡 C)**：
  - **三端 Header 品牌連結**：點擊 Header 的 Logo 即可返回各自的首頁 ( `/calculator`, `/diary`, `/institution` )。
  - **四包錢政府管道導流防呆**：修正四包錢的政府管道外部連結，點擊時彈出防呆說明彈窗，確認後才開新分頁跳轉，保障長輩瀏覽體驗。
  - **照護日誌入口優化**：`/diary` 新增 `DiaryHero` 橫幅，加強「輸入邀請碼」與「獨立記錄」的使用引導。
  - **CMS 等級評估體驗升級**：完成試算後自動鎖定結果與按鈕，防止長輩誤觸；新增「重新評估」按鈕及警告彈窗，確認後清除紀錄並返回首頁。
  - **移除家屬端多餘介面**：移除家屬端未連動時的「預估額度」分頁，專注於記錄功能。

- 🚀 **飛輪串接修正 (任務卡 B)**：
  - **試算結果頁引導**：於 `/calculator` 總覽表底部新增「開始記錄照護日誌」引導卡片，點擊可直接跳轉至 `/diary`。
  - **步進器防呆**：`/calculator` 頂部步驟條現在會嚴格檢查是否已完成測驗，未完成前鎖定「補助試算」與「總覽表」以防空白狀態。
  - **家屬端連動擴充**：`/diary` 中的「重新試算」及「用試算平台先估新等級」按鈕全面接通 `/calculator` 路由；「聯繫個管 1966」加入 `<a href="tel:1966">` 支援一鍵撥打。
  - **機構端邀請彈窗**：`/institution` 頂端新增「邀請家屬連動」按鈕，實作複製邀請碼功能，並附上家屬日誌隱私聲明。
