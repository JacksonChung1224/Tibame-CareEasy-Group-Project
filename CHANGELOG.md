...
### Changed
- 🚀 **裁決交叉路徑金額落實 (任務卡 J)**：
  - **補齊人員**：`no_schedule` 若裁決為「採紙本」，強制出現「服務人員身分證」必填框，未填妥將阻擋匯出並提示。
  - **合計修復**：整個匯出合計與步驟 4 的小計邏輯，現已改由統一的 `resolveAmount` 接管，金額計算絕對精準。
  - **取消靜默丟列**：移除了原本匯出邏輯中會把部分紀錄偷偷丟掉的問題，改交由最上層的 `getExportBlockers` 嚴格把關。
  - **即時試算反映**：步驟 3 表格裡的單價與小計會即時跟隨裁決切換更新。

- 🚀 **裁決無預設化 (任務卡 I)**：
  - **移除預設立場**：所有衝突狀態（D1, no_schedule, D3, D4）的裁決選項（採紙本 / 採系統）一律改為無預設值，強制督導逐筆人工決策。
  - **UI 與匯出卡控**：針對未裁決的紀錄，UI 顯示「⚠ 待裁決」與琥珀色警示框線；匯出功能全面封鎖，直到所有的待裁決筆數、缺失的單價與時段、以及 D1 的未執行原因皆補齊為止。
  - **金額即時預覽**：裁決切換時，畫面上的單價與小計會即時依照所選的（排班側或紙本側）數值刷新，並落實寫入官方申報檔。
  - **規則文件 v4 更新**：明確規範所有差異狀態皆為「無預設・必選」，強調每一筆應核銷金額皆須經人工確認。
...
### Added
- ✨ **裁決 UI 簡化與服務人員自動帶入 (任務卡 M)**：
  - **D3/D4 狀態列優化**：移除了容易讓使用者誤會的「備註」手動輸入框，現在僅保留「採系統/採紙本」的選項，差異資訊改為唯讀顯示，確保不會讓督導誤以為可直接修改系統金額。
  - **no_schedule 人員自動帶入**：當發生排班表中沒有，但紙本有紀錄 (`no_schedule`) 時，不再要求手動輸入「服務人員身分證」。系統會自動從同日同案的排班表或整份排班表的第一個人員抓取身分證，並在 UI 上唯讀標示「自動帶入」。
  - **匯出防呆邏輯更新**：配合自動帶入機制，匯出的 `missingWorker` 阻擋規則更新為「僅在排班檔完全沒有任何人且該紀錄也無人員」時才阻擋匯出，減少不必要的防護卡關。

- ✨ **AI 就醫摘要真實化 (任務卡 K)**：
  - **API 路由建立**：新增 `/api/summary` server route，實作與 Gemini 模型的對接，將前端發送的結構化日誌（去識別化）轉換為適合醫師閱讀的 150 字內就醫摘要。
  - **智慧防呆與降級 (Fallback)**：實作了 8 秒 Timeout 以及金鑰防呆機制，當 API 呼叫失敗或逾時，前端不會拋出任何錯誤，而是無縫地降級顯示原有的條列式模板摘要。
  - **前端互動優化**：點擊「產生就醫摘要」時，新增 `AI 整理中…` loading 狀態；完成後明確標示「由 AI 依觀察紀錄彙整」及免責聲明。

- ✨ **法務與隱私規範完善 (任務卡 L)**：
  - **全站 Footer 實作**：在三端共用版面底部新增「© 2026 CareEasy 照護一點通｜隱私權政策｜服務條款｜聯絡我們」連結列。
  - **隱私權與服務條款彈窗**：點擊 Footer 連結可開啟對應的法務彈窗（`LegalModals.jsx`），包含去識別化、大數據應用、及特種個資處理等符合法規要件的完整聲明。
  - **試算問卷連動**：問卷最後的「資料使用說明」同意區塊改為超連結，可直接開啟隱私權政策彈窗供使用者檢閱詳細內容。

- ✨ **方案說明頁與示意註冊 (任務卡 F1-Lite)**：
  - 建立 `/plans` 靜態頁，說明免費版（目前方案）與進階版（即將推出）特色。
  - 在 `FamilyDiaryV3.jsx` 日誌頁面（Solo 模式且未註冊時）新增 Email 註冊引導卡，並實作純前端的 Regex 格式檢核，全程不觸發後端 Auth 網路請求。
  - Dev 開發者切換器自動跳過註冊引導卡，提高展示便利性。
  - 於試算端 `SubsidyCalculator.jsx` CTA 區塊新增前往 `/plans` 的連結。
- ✨ **個資蒐集告知與同意 (任務卡 P)**：
  - 於 `AssessmentQuiz.jsx` 送出資料庫前最後一個步驟新增「資料使用說明」區塊。
  - 實作防呆 Checkbox，預設為未勾選。
  - 使用者若未勾選同意而嘗試點擊按鈕，會透過透明 Overlay 攔截點擊並顯示紅色提示文字，確保個資合規。

### Fixed (Hotfixes)
- 🐛 **家屬端 DATES 崩頁修正**：修復 `FamilyDiaryV3.jsx` 第 624 行 `DATES` 變數未定義的問題，同步更新為 `[...customDates, ...LAST_14]`，解決「居服紀錄」Tab 崩頁。
- 🐛 **後台試算紀錄精確度與時區寫入修復**：
  - **範圍級距漏寫修復**：當推估結果為區間（如 `2-3`）時，將完整上下限與 `isRange` 寫入 `answers` JSON 中，並更新後台管理員報表 (`admin/page.js`)，不再僅顯示單一最小值。
  - **跨瀏覽器時間格式修復**：將寫入 Supabase 的 `created_at` 從 `tw_time` 改用標準的 `new Date().toISOString()`，避免舊版 Safari 等瀏覽器因為 `sv-SE` 格式相容性導致的靜默寫入失敗。

- 🐛 **匯出鏈端到端重大修正 (任務卡 E-Hotfix)**：
  - **[H1] 時段空白阻擋機制**：修正官方申報檔必填之時段欄位缺失。OCR 時段優先，若缺則降級尋找排班時段；若皆無，則於步驟三觸發 `⚠ 缺時段` 警告並阻擋匯出，同時於步驟 2.5 開放時段欄位供人工補齊。
  - **[H2] 單價警示基準**：單價警示不再只以排班價比對附表。已修正為以「最終實際匯出之金額」作為比對基準，解決紙本價異常而排班正常時的漏報問題。
  - **[H3] D2 資訊強制填寫**：D2（計畫外服務）確認時不再寫死預設值。現已升級為專屬 Modal，強制核銷人員填寫「服務類別（自費/補助）」與「服務人員身分證字號」。
  - **[H4] Fallback 金額回寫**：若價格退回套用 `BA_MAP` 之衛福部預設金額，現會正確反映在官方申報檔輸出欄位，並在異常附表中自動加註 `單價採衛福部附表預設值`。
  - **[H5] 迴歸測試防護網**：於 `reconcile.test.mjs` 中擴充了同日剩餘不瞎猜 (2:1)、D4 觸發條件、H1 與 H2 的邊界防護測試案例。
...
### Changed
- 🚀 **機構端核銷體驗純化 (任務卡 F)**：
  - **匯出純化與防呆**：修復匯出崩潰，移除多餘的 `hoursDerived`，確保符合 11 欄格式，並防止缺價列進入報表。
  - **D2 退場與 no_schedule**：移除 D2 強制填寫 Modal，改標為 `no_schedule`，且不阻擋匯出、不列入核銷明細。
  - **D3/D4 裁決介面**：新增「採紙本 / 採系統」的 radio 按鈕，預設採紙本，並於底層實作 `resolveRow` 動態對齊匯出值。
  - **OCR 體驗精簡與升級**：從步驟 2 移除生命徵象欄位與單價警示，並實作原生 `<input type="file" capture="environment">` 提供高質感的照片疊影預覽。
  - **個案連動狀態擴充**：個案管理頁新增了「本月已確認服務紀錄」的展開表格與推播家屬端按鈕。
- 🚀 **家屬端照護日誌日期升級 (任務卡 G)**：
  - **動態相對日期**：引入 `fmtMD` 與 `daysAgo` 將所有靜態 Mock 日期與 14 天陣列動態化，永遠以今天為基準往前推算。
  - **原生日期選擇器**：日期列最右側新增 📅 原生 `<input type="date">` 選擇器，選取非 14 天內的歷史日期時會自動標示「自選」並安插於陣列最前端。
  - **「今天」視覺高亮**：替今天的日期 chip 加上專屬標籤與淡色外框以供辨識。
- 🚀 **手寫觀察資料流與裁決統一 (任務卡 H)**：
  - **手寫觀察雙流並行**：於 OCR 確認步驟新增「個案特殊狀況（手寫辨識）」區塊，支援低信心度警示與編輯。手寫觀察確認後獨立存入個案連動管理，完全不阻擋且不影響核銷流程，體現「一筆紙本紀錄，三種價值」。
  - **個案連動管理擴充**：於個案管理的「本月已確認服務紀錄」下方增設「📝 照護觀察（居服員手寫紀錄）」展示區塊，推播至家屬端時亦會涵蓋該觀察內容，精確命中家屬端 AI 訊號分析系統（如「跌倒」關鍵字）。
  - **裁決語義與預設統一**：將 `D1` (疑似未執行) 與 `no_schedule` (紙本有排班無) 的處理介面統一為「Radio + 備註」版型。D1 預設採紙本（未執行、不入明細、備註必填），`no_schedule` 預設採系統（不入明細），並升級底層 `resolveRow()` 引擎精確控制匯出行為。
  - **規則文件升級 (v3)**：將上述新裁決機制語義及預設值寫入 `reconciliation-rules.md` v3 並標註由 CMS Rules Specialist 簽核完成。
